...
function findClosest(from, target, direction) {
  //from: id, target: link array, direction (optional) up, down, left, right
  try {
    ...
    $.each(target, function (i, el) {
      // find closest menu item, compares center coordinates
      if ($(el).attr("id") == from) return; // do not count itself
      var offset = $(el).offset();
      var cx2 = $(el).width() / 2;
      var cy2 = $(el).height() / 2;
      pos.b = {
        x: Math.round(offset.left + cx2),
        y: Math.round(offset.top + cy2),
      };
      ...
      if (direction) {
        if (
          (direction == "up" && pos.b.y >= pos.a.y + Math.abs(pos.b.x - pos.a.x)) ||
      ...
      var dist = Math.sqrt(
        Math.pow(offset.left + cx2 - from_offset.left - cx, 2) + Math.pow(offset.top + cy2 - from_offset.top - cy, 2)
      );
      if (dist < shortest) {
        shortest = dist;
        index = el;
