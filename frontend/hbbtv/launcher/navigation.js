...
function registerKeys() {
  ...
  try {
    ...
    try {
      ...
      cfg.configuration.replaceUIElements([
        cfg.configuration.UI_TVMODE,
        cfg.configuration.UI_MENU,
  ...
  if (!animating) {
    ...
    switch (keyCode) {
      ...
      case VK_DOWN:
        ...
        if (fastScrolling) {
          if (firstPressed) firstPressed = false;
          if (!scrolling) {
            onKey(VK_FAST_FWD);
          }
...
function showInfobanner() {
  ...
  if (channel.epg) {
    ...
    supervisor &&
    serviceInstance &&
    supervisor.currentChannel &&
    serviceInstance.dvbChannel &&
    supervisor.currentChannel.ccid == serviceInstance.dvbChannel.ccid
  ) {
      ...
      openMenu.childNodes.getByClass("items")[0].removeClass("visible", function () {
        nextMenu.childNodes.getByClass("items")[0].removeClass("visible", function () {
          ...
          menuTimer = setTimeout(function () {
            ...
            updateTimer = setTimeout(function () {
              updateOpenChannel();
              nextChannel.getCenterBox().autoScrollChildren();
            }, updateTime);
          }, menuTime);
      ...
      previousMenu.childNodes.getByClass("items")[0].removeClass("visible", function () {
        openMenu.childNodes.getByClass("items")[0].removeClass("visible", function () {
          ...
          menuTimer = setTimeout(function () {
            ...
            updateTimer = setTimeout(function () {
              updateOpenChannel();
              previousChannel.getCenterBox().autoScrollChildren();
            }, updateTime);
          }, menuTime);
  ...
  try {
    if (_box instanceof Box) {
      if (focusElement(_box.element)) {
        ...
        if (bufferScrollingTimer != null) {
          clearTimeout(bufferScrollingTimer);
          bufferScrollingTimer = null;
        }
