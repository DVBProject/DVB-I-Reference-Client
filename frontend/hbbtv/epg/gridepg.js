...
GridEPG.prototype.loadPreviousDay = function(callback){
	console.log("GridEPG.loadPreviousDay");
	try{
		setLoading(true);
		// Load previous day
		var day = new Date(this.firstDay.getTime() - TWENTY_FOUR_HOURS);
		var daystr = day.getFullYear()+addZeroPrefix(day.getMonth()+1)+addZeroPrefix(day.getDate());
		var epg = this;
		getData(daystr, cids.toString(), 1, epglang, function(response){
			var emptyEPG = true;
			for(var i = 0; i < response["channels"].length; i++){
				if(response["channels"][i]["epg"].length > 0){
					emptyEPG = false;
					break;
				}
			}
			...
			if(!emptyEPG){
				var ch = (epg.channels.indexOf(epg.getOpenChannel()) >= 0) ? epg.channels.indexOf(epg.getOpenChannel()) : 0;
				epg.firstDay = day;
				epg.lastDay = new Date(epg.firstDay.getTime() + ((epg.days-1) * TWENTY_FOUR_HOURS));
				epg.timelineend = new Date(epg.timelineend.getTime() - (1000 * 60 * 30));
				epg.timelinestart = new Date(epg.timelinestart.getTime() - (1000 * 60 * 30));
				epg.insertFirstDay(response);
				epg.removeLastDay();
				epg.populatePrograms();
				epg.channels[ch].setFocus();
				setLoading(false);
			}
			else{
				epg.previousDayIsAvailable = false;
				setLoading(false);
			}
...
GridEPG.prototype.loadNextDay = function(callback){
	console.log("GridEPG.loadNextDay");
	try{
		setLoading(true);
		// Load previous day
		var day = new Date(this.lastDay.getTime() + TWENTY_FOUR_HOURS);
		var daystr = day.getFullYear()+addZeroPrefix(day.getMonth()+1)+addZeroPrefix(day.getDate());
		var epg = this;
		getData(daystr, cids.toString(), 1, epglang, function(response){
			var emptyEPG = true;
			...
			if(!emptyEPG){
				var ch = (epg.channels.indexOf(epg.getOpenChannel()) >= 0) ? epg.channels.indexOf(epg.getOpenChannel()) : 0;
				epg.lastDay = day;
				epg.firstDay = new Date(epg.lastDay.getTime() - ((epg.days-1) * TWENTY_FOUR_HOURS));
				epg.timelineend = new Date(epg.timelineend.getTime() + (1000 * 60 * 30));
				epg.timelinestart = new Date(epg.timelinestart.getTime() + (1000 * 60 * 30));
				epg.removeFirstDay();
				epg.insertLastDay(response);
				epg.populatePrograms();
				epg.channels[ch].setFocus();
				setLoading(false);
			}
			else{
				epg.nextDayIsAvailable = false;
				setLoading(false);
			}
