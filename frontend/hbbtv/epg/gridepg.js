...
GridEPG.prototype.initEPG = function (
  element_id,
  epgdata,
  firstvisible_channel,
  visible_channels,
...
GridEPG.prototype.initCurrentTimeline = function () {
  if (document.getElementById("currentTimelineContainer") == null) {
    ...
    var pos =
      ((curTime.getTime() - this.timelinestart.getTime()) / 1000 / 60) * MINUTE_WIDTH -
      this.currentTimeline.offsetWidth / 2 -
      vtimeline.offsetWidth / 2;
  ...
  if (program instanceof Program) {
    try {
      ...
      if (program.noprogram) {
        ...
        if (
          program.start_date_obj.getTime() <= curTime.getTime() &&
          curTime.getTime() <= program.end_date_obj.getTime()
        ) {
          minsLeft = Math.round((program.end_date_obj.getTime() - curTime.getTime()) / 60000);
        }
        program_time.innerHTML = program.start_date_obj.format("H:i") + " - ";
        ...
        description_text.innerHTML = program
          .getSynopsisText()
          .replace(/&/g, "&amp;")
          .replace(/</g, "&lt;")
          .replace(/>/g, "&gt;");
      ...
      program.getChannel().getProgramInfo(program.programId, function (info) {
        if (info) {
          var i,
            longDesc = info.getLongDescription();
          if (longDesc) {
          ...
          if (info.creditsItems) {
            for (i = 0; i < info.creditsItems.length; i++) {
              ...
              if (info.creditsItems[i].character) {
                extendedData +=
                  "(" +
                  info.creditsItems[i].character.givenName +
                  " " +
                  info.creditsItems[i].character.familyName +
                  ")";
              }
              extendedData += "<br/>";
            }
      ...
      var imageurls = [];
      if (program["serieimage"]) {
        imageurls.push(program["serieimage"]);
      }
      if (program["mediaimage"]) {
        imageurls.push(program["mediaimage"]);
      }
      if (program["mediaimage2"]) {
        imageurls.push(program["mediaimage2"]);
      }
      if (program["channelimage"]) {
        imageurls.push(program["channelimage"]);
      }
      var imageurl_idx = 0;
...
GridEPG.prototype.autoScrollDescriptionText = function () {
  ...
  self.timeouts.autoScrollDescriptionTimeouts.push(
    setTimeout(function () {
...
GridEPG.prototype.autoScrollDetailProgramTitle = function () {
  ...
  if (scrollDistance > 0) {
    ...
    self.timeouts.autoScrollDetailProgramTitleTimeouts.push(
      setTimeout(function () {
        $(element).animate(
